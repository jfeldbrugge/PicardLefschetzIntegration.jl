...
# ╔═╡ 9d0e0abf-18a3-4a50-8d47-8e18684f3a70
using PicardLefschetzIntegration, ProgressMeter, Base.Threads, Alert, CairoMakie, GeometryBasics, PlutoUI

# ╔═╡ 2ef58f9a-347e-429c-a33a-d67649f56539
md"""
# Two-dimensional Gaussian lens
"""

# ╔═╡ ae6c5018-1b98-4870-9002-122b2477ab13
md"""
We evaluate the Kirchhoff-Fresnel integral for the two-dimensional Gaussian lens

$\require{bm}$
$\Psi(\boldsymbol{y}) = \int_{-\infty}^\infty\int_{-\infty}^\infty e^{i \omega \left[
\frac{(\boldsymbol{x} - \boldsymbol{y})^2}{2} + \varphi(\boldsymbol{x})
\right]}\mathrm{d}x_1 \mathrm{d}x_2$

with the phase-variation 

$\varphi(\boldsymbol{x}) = e^{-\frac{x_1^2 + 2 x_2^2}{2}}$

with $\boldsymbol{x} = (x_1,x_2)$.
"""

# ╔═╡ 550b7a50-697f-4028-a091-873ab5e752ca
pars = parameters(δ = 1., τ = -10., ϵ = 0.1, N = 20, n = 8, dim = 2)

# ╔═╡ bddafe13-78df-495c-a76e-3e1c0ce53074
φ(p) = exp(- (p[1]^2 + 2 * p[2]^2) / 2)

# ╔═╡ a486e3ba-e0ae-4b4d-92d5-3bb78c54da9e
S(p, x₁, x₂, ω) = ω * (((p[1] - x₁)^2 + (p[2] - x₂)^2) / 2. + φ(p))

# ╔═╡ 8fd0ccbd-7ab9-46e5-b05f-512f1a1a0499
md"""
## Evaluate thimbles
"""

# ╔═╡ 15c06154-147f-4fab-9519-8039f9b465bb
aRange_L, bRange_L = range(-1., 1., 10), range(-1., 1., 10);

# ╔═╡ e2f332d1-9ea6-4aba-b867-e741246ebb70
thimbles = Array{thimble}(undef, length(aRange_L), length(bRange_L));

# ╔═╡ 2088d05d-69a2-40ff-b7d3-148784561d84
let ω = 1
    Threads.@threads for (i, j) in collect(Iterators.product(eachindex(aRange_L),eachindex(bRange_L)))
...
# ╔═╡ f8e3ab82-b849-44ac-9e3a-9979ae2d1f01
map(t->length(t.simplices), thimbles)

# ╔═╡ 727f3897-afe0-4dee-99a2-8657a2fd6eb6
md"""
## Evaluate integral
"""

# ╔═╡ 9b25a2d0-8e9c-40d4-93e9-a8eb31d134a9
aRange, bRange = range(-1., 1., 100), range(-1., 1., 100);

# ╔═╡ 86ca19ab-f14f-45ed-9a51-e42f123c3a81
let ω = 10
...
# ╔═╡ c21be022-0032-4a16-979a-0de475881c06
let ω = 25
    data = zeros(Complex, length(aRange), length(bRange));
    Threads.@threads for (i, j) in collect(Iterators.product(eachindex(aRange),eachindex(bRange)))
        x₁, x₂ = aRange[i], bRange[j]
...
# ╔═╡ f3a7f374-dcb3-4449-a756-5345a71e9ade
let ω = 50
    data = zeros(Complex, length(aRange), length(bRange));
    Threads.@threads for (i, j) in collect(Iterators.product(eachindex(aRange),eachindex(bRange)))
        x₁, x₂ = aRange[i], bRange[j]
...
# ╔═╡ 288e2128-4636-44dd-b79f-23307afe9eeb
let ω = 75
    data = zeros(Complex, length(aRange), length(bRange));
    Threads.@threads for (i, j) in collect(Iterators.product(eachindex(aRange),eachindex(bRange)))
        x₁, x₂ = aRange[i], bRange[j]
...
# ╔═╡ 64408070-4984-4090-a0fa-b7bad59bb37d
let ω = 100
    data = zeros(Complex, length(aRange), length(bRange));
    Threads.@threads for (i, j) in collect(Iterators.product(eachindex(aRange),eachindex(bRange)))
        x₁, x₂ = aRange[i], bRange[j]
...
PicardLefschetzIntegration = "86202789-ec99-4348-b84a-34e9b1fc693e"
...
PicardLefschetzIntegration = "~1.0.0"
PlutoUI = "~0.7.83"
...
manifest_format = "2.0"
project_hash = "091b655d7117b89cdd98f120ec94173255bfea40"
...
[[deps.Alert]]
deps = ["Base64", "Dates", "Logging", "MacroTools", "Printf", "REPL"]
git-tree-sha1 = "7023de7c90905271b1b61149b133905f5a1f4bb4"
...
[[deps.PicardLefschetzIntegration]]
deps = ["Combinatorics", "LinearAlgebra", "SimplexQuad"]
git-tree-sha1 = "a5d1c8c8d0ac30d132444f177d24a9ce7210ac0b"
uuid = "86202789-ec99-4348-b84a-34e9b1fc693e"
version = "1.0.0"
...
git-tree-sha1 = "388cbe3a2afb2cb278ff6e022f54c1ba3ecd1036"
uuid = "f5de1e94-2f77-472d-8c3b-9d09f580ee5e"
version = "1.0.1"
