...
# Theory
Picard-Lefschetz theory formalizes the famous saddle point method for multidimensional integrals. In the process, it provides a systematic method to evaluate highly oscillatory integrals. We will here briefly describe Picard-Lefschetz theory and the implementation of the present package.
...
A saddle point is relevant, *i.e*, the descent manifold $\mathcal{J}_j$ assoicated with the critical point $\boldsymbol{x}_j$ contributes to the deformation, when the intersection number $n_j \in \mathbb{Z}$ is non-zero. When $n_j \neq 0$ the critical point $\boldsymbol{x}_j$ is relevant to the integral. When $n_j=0$ the critical point $\boldsymbol{x}_j$ is irrelevant. Remarkably, the intersection number $n_j$ of the critical point $\boldsymbol{x}_j$ is given by the number of times its associated steepest ascent manifold $\mathcal{K}_j$ intersects the original integration domain $D$, *i.e.*, $n_j = \langle D, \mathcal{K}_j\rangle$. This deformation is optimal as the Cauchy-Riemann equations guarantee that the integrand does no-longer oscillate along the steepest descent manifold.

Given an oscillatory integral, its evaluation has been reduced to computing the computation of the intersection numbers $n_j$ and the associated relevant steepest descent manifolds $\mathcal{J}_j$.
...
known as the one-dimensional Lorentzian lens model. In this integral depends on the frequency $\omega$ and the time delay function 
